...
export default class Circle {
  ...
  transformScreenToWorld(currentPx: vec2, x: number, y: number) {
  ...
  isInside(resp:vec2, currentPx:vec2 ,x: number, y: number): boolean {
    currentPx = this.transformScreenToWorld(currentPx, x, y);

    // gl-matrix를 통한 길이 연산
    // const leng = vec2.length(vec2.subtract(resp, currentPx, this.circleAttrs.center));

    const dx = this.circleAttrs.center[0] - currentPx[0];
    const dy = this.circleAttrs.center[1] - currentPx[1];
