...
export class PerspectiveRenderer implements PerspectiveRenderStrategy {
  ...
  trayceRay(start: vec3, rayDir: vec3) {
    ...
    if (hit?.obj instanceof Triangle) {
      const color1 = vec3.fromValues(1, 0, 0);
      const color2 = vec3.fromValues(0, 1, 0);
      const color3 = vec3.fromValues(0, 0, 1);
      const { w0, w1 } = hit;
      if (w0 !== undefined && w1 !== undefined) {
        ...
        return vec3.add(
          vec3.create(),
          vec3.scale(vec3.create(), color1, w0),
          vec3.add(
            vec3.create(),
            vec3.scale(vec3.create(), color2, w1),
            vec3.scale(vec3.create(), color3, w2)
          )
        );
      }
