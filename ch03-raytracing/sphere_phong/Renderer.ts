...
function vec3Pow(out: vec3, v: vec3, power: number): vec3 {
  out[0] = Math.pow(v[0], power);
  out[1] = Math.pow(v[1], power);
  out[2] = Math.pow(v[2], power);
  return out;
}

export class PhongRenderer implements RenderStrategy {
  render(sphere: Sphere, canvas: Canvas) {
    canvas.clear();
    const imageData = canvas.getImageData();
    const data = imageData.data;
    ...
    for (let x = 0; x < canvasWidth; x += 1) {
      for (let y = 0; y < canvasHeight; y += 1) {
        const { wX, wY, wZ } = this.transformScreenToWorld(x, y, canvasWidth, canvasHeight, canvas.getAspectRatio());
        const hit = this.trayceRay(vec3.set(currentPx, wX, wY, wZ), rayDir, sphereAttrs);

        if (hit) {
          const { dist, phong } = hit;
          // data[index] = color[0] * 255 * dist;
          // data[index + 1] = color[1] * 255 * dist;
          // data[index + 2] = color[2] * 255 * dist;
          if (phong) {
            ...
            data[index] = phong[0] * 255;
            data[index + 1] = phong[1] * 255;
            data[index + 2] = phong[2] * 255;
            data[index + 3] = 255;
          }
        }
      }
  ...
  trayceRay(start: vec3, rayDir: vec3, sphereAttrs: SphereAttrs): Hit | null {
    const { center, radius, ambient, diffusion, light, ks, specular, alpha } = sphereAttrs;
    const o_c = vec3.subtract(vec3.create(), start, center);
    ...
    const nabla = b * b - c_;
     ...
     if (nabla >= 0) {
      ...
      const dirToLight = vec3.normalize(
        vec3.create(),
        vec3.subtract(vec3.create(), light, hitPoint)
      );
      ...
      const phong = vec3.add(vec3.create(), vec3.add(vec3.create(), ambient, diffuse), _specular);
