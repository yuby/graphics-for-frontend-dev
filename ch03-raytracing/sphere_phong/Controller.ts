...
export default class Controller {
  private center = vec3.fromValues(0, 0, 0.5);

  private light = vec3.fromValues(0, 0, -1);

  private color = vec3.fromValues(1, 1, 1);

  private ambient = vec3.fromValues(0, 0, 0);
  private diffusion = vec3.fromValues(0, 0, 1);
  private specular = vec3.fromValues(1, 1, 1);
  private alpha = 10;
  private ks = 0.8;
  ...
  attach() {
    ...
    const alphaControl = this.createControl('Specular_Power', 0, 100, this.alpha);
    container.appendChild(alphaControl);

    const ksControl = this.createControl('Specular_Coefficient', 0, 1, this.ks);
    container.appendChild(ksControl);


    const lightWrap = this.warpControl();
    const lightRControl = this.createControl('Light_X', -10, 10, this.light[0]);
    lightWrap.appendChild(lightRControl);
    const lightGControl = this.createControl('Light_Y', -10, 10, this.light[1]);
    lightWrap.appendChild(lightGControl);
    const lightBControl = this.createControl('Light_Z', -10, 10, this.light[2]);
  ...
  createControl(label: string, min: number, max: number, value: number) {
    ...
    input.addEventListener('input', (e: Event) => {
      ...
      if (label === 'Specular_Power') this.alpha = Number(target.value);
      if (label === 'Specular_Coefficient') this.ks = Number(target.value);
      if (label === 'Light_X') this.light[0] = Number(target.value);
      if (label === 'Light_Y') this.light[1] = Number(target.value);
      if (label === 'Light_Z') this.light[2] = Number(target.value);
