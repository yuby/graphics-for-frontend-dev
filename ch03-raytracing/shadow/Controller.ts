...
import { ObjectAttrs } from './MyObject';

interface ControllerReturnProps extends ObjectAttrs, SphereAttrs {}

export default class Controller {
  constructor(
    private obj: Sphere,
    private callback?: (attrs: ControllerReturnProps) => void
  ) {
    this.attach();
  }
  ...
  createControl(label: string, min: number, max: number, value: number) {
    ...
    input.addEventListener('input', (e: Event) => {
      ...
      if (this.callback) this.callback(this.obj.getAttrs());
    });
