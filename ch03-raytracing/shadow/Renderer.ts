...
export interface PerspectiveRenderStrategy {
  ...
  setObjects(objects: MyObject[]): void;
...
export class PerspectiveRenderer implements PerspectiveRenderStrategy {
  ...
  findClosestCollision(start: vec3, rayDir: vec3): Hit | null {
    ...
    this.objects.forEach((obj) => {
      if (obj.checkRayCollision) {
        const hit = obj.checkRayCollision(start, rayDir);
  ...
  trayceRay(start: vec3, rayDir: vec3) {
    ...
    if (hit && hit.dist >= 0) {
      ...
      const shadowRayStart = vec3.add(vec3.create(), hit.hitPoint, vec3.scale(vec3.create(), dirToLight, 0.001)); 
      const shadowRayDir = dirToLight;

      const hitShadow = this.findClosestCollision(shadowRayStart, shadowRayDir);

      if (!hitShadow || hitShadow.dist < 0) {
